...
/// RTPS Entity represents the class of objects that are visible to other RTPS Entities on the network. As such, RTPS Entity objects have a globally-unique identifier (GUID) and can be referenced inside RTPS messages.
trait Entity {
    fn entity(&self) -> Guid;
}
...
/// Must be possible to represent using 12 octets. The following values are reserved by the protocol: GUIDPREFIX_UNKNOWN
pub(crate) type GuidPrefix = [u8; 12];

/// Type used to hold the suffix part of the globally-unique RTPS-entity identifiers. The EntityId_t uniquely identifies an Entity within a Participant. Must be possible to represent using 4 octets.
...
pub(crate) type EntityId = [u8; 4];

/// Type used to hold sequence numbers. Must be possible to represent using 64 bits.
/// The following values are reserved by the protocol: SEQUENCENUMBER_UNKNOWN
pub(crate) type SequenceNumber = i64;
